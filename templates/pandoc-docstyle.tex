...
\usepackage{templates/myElectricalTokens}		% For the Grounding symbol
\usepackage[en-GB,datesep={-}]{datetime2}
%\usepackage{longtable}
\usepackage{xltabular}					% Includes longtable with the column specifier X as in tabularx
\usepackage{caption}						% For placing labels in a floating environment (specifically, multiple design principles in a table)
\usepackage[within=none]{newfloat}					% For specifying a new foating environment (specifically, design principles)
\usepackage{gensymb}						% Generic macros to typeset units of measurements independent on text or math modes. See http://ctan.cs.uu.nl/macros/latex/contrib/was/gensymb.pdf
...
\DeclareFloatingEnvironment[listname={List of Design Principles}]{DesignPrinciple}
...
\newtheorem{mmh}{Heuristic}[chapter]
...
\theoremstyle{}		% The header for all theorems are followed by a newline
...
\begin{document}
% Set properties of particular packages
% caption
\captionsetup[DesignPrinciple]{margin=10pt,font=small,labelfont=bf}
...
% --- Keywords --- --- --- --- ---
$if(kw)$		% use pandoc option -M KEY[=VAL]	
\begin{keywords}
$tags$
\end{keywords}
$endif$
...
$if(todo)$		% use pandoc option -V todo
\listoftodos
\pagebreak
$endif$
